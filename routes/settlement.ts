...
import { generatePromptPayPayload } from "../services/settlement.service";
...
export const settlementRoutes = new Elysia()
  .get(
    ...
    async ({ params: { code }, set }) => {
      ...
      // userId -> phone (เบอร์ PromptPay ของเจ้าหนี้ ไว้ generate QR)
      ...
      const transactions: {
        ...
        amount: number;
        toPhone: string | null;
        promptPayPayload: string | null; // fix amount = ยอดใน transaction นี้
      }[] = [];
      ...
      while (ci < creditors.length && di < debtors.length) {
        ...
        const roundedAmount = Math.round(amount * 100) / 100; // ปัด 2 ตำแหน่ง
        const toPhone = phoneById.get(creditors[ci].userId) ?? null;
        transactions.push({
  ...
  .post(
    ...
    },
    {
      ...
      detail: {
        summary: "QR Code สำหรับการหารเงิน",
        description: "สร้าง QR Code สำหรับการหารเงิน",
        tags: ["Settlement"],
      },
    },
  );
